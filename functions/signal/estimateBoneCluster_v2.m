function [f, data_cleaned, cluster_idx] = estimateBoneCluster_v2(peaks, seed, clusterparam_dim, outlierparam_gradthresh, outlierparam_minpoint, display)
...
if( abs(max(gradients(:,2))-min(gradients(:,2))) > 1e-7 )
    % calculate the mean of the gradient to define outlier threshold
    gradients_thresh = mean(gradients(:,2)) * outlierparam_gradthresh;
...
if(~isempty(data_cleaned))
    f = fit( data_cleaned(:,1), ...
             data_cleaned(:,2), ...
             'smoothingspline','SmoothingParam',0.0035);
else
    f = [];
end
