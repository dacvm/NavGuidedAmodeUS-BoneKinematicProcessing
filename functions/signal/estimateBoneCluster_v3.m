function [f, data_cleaned, cluster_idx, figure1] = estimateBoneCluster_v3(peaks, seed, clusterparam, outlierparam, smoothingparam, display)
...
if(isempty(clusterparam))
    clusterparam.epsilon = 0.1;
    clusterparam.minpts = 40;
    clusterparam.cov = [300 0; 0 1.0];
end
cluster_idx = dbscan(data_norm, clusterparam.epsilon, clusterparam.minpts, 'Distance', 'Mahalanobis', 'Cov', clusterparam.cov);
...
% 2.1) If seed is empty, sort the cluster and pick the latest cluster 
if (isempty(seed))

    % 2.1.2) Based on the mean we obtaned, we sort our cluster ------------
    ...
    % 2.1.3) Get the latest cluster, we assumed it is the bone ------------
    
    % get the unique labels
    cluster_labels    = unique(cluster_idx);
    % bone is the deepest cluster, but we need to throw the possibility if the
    % deepst cluster is just noises, so let's make a filter here. 
    offset = 0;
    for i=1:cluster_n
        cluster_select    = cluster_labels(end-offset);
        member_clusteridx = cluster_idx==cluster_select;
        % ignore the cluster if it is less than 20% from the entire frame
        if(sum(member_clusteridx) <= 0.2*peaks(end,1))
            offset = offset+1;
            continue;
        end
        member_cluster    = peaks(member_clusteridx, : );
...
% 2.2) If seed is not empty, we pick the cluster that is closest to the seed 
else
    % find clusters in which the mean is the closest to the seed
    [~, min_idx]      = min(abs(cluster_mean-seed));                        % closest the seed just by simple subtraction with mean
    cluster_select    = cluster_labels(min_idx);                            % first element of cluster_mean=0 --> cluster_mean = -1, so better with proper selection
    % get the member of that cluster
...
% only do outlier removal if the data considerately jaggy
if( abs(max(gradients(:,2))-min(gradients(:,2))) > 0.1 )
    % calculate the mean of the gradient to define outlier threshold
    gradients_thresh = mean(gradients(:,2)) * outlierparam.gradthresh;
    outliers         = [0; find(gradients(:,2)>=gradients_thresh); length(gradients)+1];

% if overall gradient is smooth, dont do outlier removal
else
    gradients_thresh = max(gradients(:,2));
    outliers         = [0; length(gradients)+1];
end
...
for i=1:length(outliers)-1
    ...
    if(length(tmp) >= outlierparam.minpoint)
...
if (display)
    ...
    % prepare window
    figure1 = figure('Name', 'Cleaning and Clustering', 'Position', [100 100 1600 600]);
    ...
    ylabel(ax_clustering, 'Depth (mm)', 'Interpreter', 'Latex');
    xlabel(ax_clustering, 'Timestamp');
    title(ax_clustering, 'Ultrasound Peaks');
    ylim(ax_clustering, [0 20]);
    ...
    yline(ax_gradient, gradients_thresh, '--r', 'Threshold', 'LineWidth', 1);
