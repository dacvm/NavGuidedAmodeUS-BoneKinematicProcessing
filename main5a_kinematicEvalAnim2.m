...
for idx_view = 1:3
    ...
    xlabel(ax1(idx_view), '← Posterior | X (mm) | Anterior →');
    ylabel(ax1(idx_view), '← Distal | Y (mm) | Proximal →');
    zlabel(ax1(idx_view), '← Medial | Z (mm) | Lateral →');
...
title(ax1(1), 'Sagital Plane (from lateral)');
view(ax1(1), [0 0 1]);
title(ax1(2), 'Transverse Plane (from top)');
view(ax1(2), [0 1 0]);
ax1(2).CameraUpVector = [1 0 0];
% put the Z axis on the edge you want (choose corners via XLim/YLim)
ax1(2).ZAxis.FirstCrossoverValue  = ax1(2).XLim(1);   % X corner  (xmin or xmax)
ax1(2).ZAxis.SecondCrossoverValue = ax1(2).YLim(1);   % Y corner  (ymin or ymax)

title(ax1(3), 'Frontal Plane (from front)');
view(ax1(3), [1 0 0]);
% ax1(3).CameraUpVector = [0 1 0];
...
for idx_dof=1:6
    ...
    plot(ax2(idx_dof), timestamp_ms_valid_select0 * 1e-3, currentDoF_kneeJoint_est, '-', 'Color', 'r', 'LineWidth', 2);
    plot(ax2(idx_dof), timestamp_ms_valid_select0 * 1e-3, currentDoF_kneeJoint_gt, '-', 'Color', 'b', 'LineWidth', 2);
...
% for idx_t = 541
    ...
    current_boneCTpoints_GT = T_bone_femur * inv(T_bone_ct) * ...
   ...
   % Display the bone and the CS on all of the 3-view axes
    for i=1:3
        % display the surface
        trisurf(current_boneCTtri_GT, 'FaceColor', '#2ecc71', 'FaceAlpha', 0.1, 'EdgeColor', '#2ecc71', 'EdgeAlpha', 0.08, 'Parent', ax1(i), 'Tag', '3d_bone_gt');
        % display the coordinate frame
        tmp = T_bone_femur;
        display_axis_v2(ax1(i), tmp(1:3, 4), tmp(1:3, 1:3), 15, '', 'Tag', 'cs_bone_gt');
    end
    ...
    r_tibia = [r_tibia(1), r_tibia(2), r_tibia(3)];

    % display the femur cs on top of tibia cs (rotation become easier to  understand and compared)
    display_axis_v2(ax1(1), t_tibia, eye(3), 15, '', 'Tag', 'cs_bone_gt', 'Mode', 'shadow', 'HideVector', [0 0 1]);
    display_axis_v2(ax1(2), t_tibia, eye(3), 15, '', 'Tag', 'cs_bone_gt', 'Mode', 'shadow', 'HideVector', [0 1 0]);    
    display_axis_v2(ax1(3), t_tibia, eye(3), 15, '', 'Tag', 'cs_bone_gt', 'Mode', 'shadow', 'HideVector', [1 0 0]);
    ...
    text( ax1(1), t_tibia(1), t_tibia(2), t_tibia(3), sprintf('(%.2f, %.2f)  ', t_tibia(1), t_tibia(2)), ...
    ...
    text( ax1(2), t_tibia(1), t_tibia(2), t_tibia(3), sprintf('(%.2f, %.2f)  ', t_tibia(1), t_tibia(3)), ...
          'HorizontalAlignment', 'right', 'VerticalAlignment', 'middle', 'Tag', 'text_t_gt');
    text( ax1(3), t_tibia(1), t_tibia(2), t_tibia(3), sprintf('(%.2f, %.2f)  ', t_tibia(2), t_tibia(3)), ...
          'HorizontalAlignment', 'right', 'VerticalAlignment', 'top', 'Tag', 'text_t_gt');

    % display the rotation text
    scaler = 20;
    text( ax1(1), t_tibia(1)+15, t_tibia(2), t_tibia(3), sprintf('%.2f°', r_tibia(1)), ...
          'HorizontalAlignment', 'left', 'VerticalAlignment', 'middle', 'Tag', 'text_r_gt');
    text( ax1(2), t_tibia(1), t_tibia(2), t_tibia(3)+15, sprintf('%.2f°', r_tibia(2)), ...
          'HorizontalAlignment', 'left', 'VerticalAlignment', 'middle', 'Tag', 'text_r_gt');
    text( ax1(3), t_tibia(1), t_tibia(2)+15, t_tibia(3), sprintf('%.2f°', r_tibia(3)), ...
    ...
    for idx_dof=1:6
        ...
        % first frame will be denoted as 0
        x_value = (timestamp_ms_valid(idx_t) - timestamp_ms_valid_select(1)) * 1e-3;

        % draw the line and current point
        xline(ax2(idx_dof), x_value, 'k', 'Tag', 'current_time');
        plot(ax2(idx_dof), x_value, kneeJoint6DOFs_estSmooth(idx_t, idx_dof), 'ro', 'MarkerFaceColor', 'r', 'MarkerSize', 10, 'Tag', 'current_est');
        plot(ax2(idx_dof), x_value, kneeJoint6DOFs_gt(idx_t, idx_dof), 'bo', 'MarkerFaceColor', 'b', 'MarkerSize', 10, 'Tag', 'current_gt');
